 ...
 *
 * Интерфейс с именем `BaseService` представляет собой шаблон (blueprint) для класса сервиса, который предоставляет операции для
 * создания, обновления, поиска и удаления объектов определенного типа `E`, где `E` должен наследовать `BaseEntity`.
 *
